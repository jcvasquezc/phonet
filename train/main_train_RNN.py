...
def generate_data(directory, batch_size, problem, mu, std):
    i = 0
    ...
    while True:
        ...
        class_weights=class_weight.compute_class_weight('balanced', np.unique(y), ystack)
...
if __name__=="__main__":
    ...
    if len(sys.argv)!=5:
        print("python main_train_RNN.py <path_seq_train> <path_seq_test> <path_results> <class_problem>")
        sys.exit()
    ...
    history=modelPH.fit_generator(generate_data(file_feat_train, batch_size, problem, mu, std), steps_per_epoch=steps_per_epoch, 
    epochs=epochs, shuffle=True, validation_data=generate_data(file_feat_test, batch_size, problem, mu, std), 
    verbose=1, callbacks=[earlystopper], validation_steps=validation_steps)
